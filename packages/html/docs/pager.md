...
| `.k-pager-nav` | `role` = `button` | Specifies the role of the navigation element. |
...
| `.k-pager-numbers .k-button` | `aria-label` / `title` | Specifies the purpose of each page link, e.g., Page 6. |
| `.k-pager-numbers .k-button.k-selected` | `aria-current` = `page` | Present on the currently selected page element. |
| `.k-pager-sizes .k-dropdownlist` | `aria-label` / `aria-labelledby` | The DropDownList requires a label to specify its purpose. |
