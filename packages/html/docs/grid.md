...
| `.k-grid-toolbar` | `aria-controls` = `.k-grid-aria-root id` | Pointing to the id of the element with role=grid. |
...
| `.k-grouping-header` | `aria-controls` = `.k-grid-aria-root id` | Pointing to the id of the element with role=grid. |
...
| `.k-grid-aria-root` | `aria-colcount` | The total number of columns in the Grid. Needed for Virtual columns and Hidden columns scenarios. |
| `.k-grid-aria-root` | `aria-rowcount` | The total number of rows in the table. Needed for Paging, Virtual rows, or Master/Detail rows scenarios. |
...
| `.k-grid-header-wrap>table>thead>tr:not(.k-filter-row)` | `aria-rowindex` | Row number. Needed for Paging, Virtual rows, or Master/Detail rows scenarios. |
...
| `.k-grid-header-wrap>table>thead>tr:not(.k-filter-row)>th` | `aria-sort` | Present if sorting is enabled for that column. |
| `.k-grid-header-wrap>table>thead>tr:not(.k-filter-row)>th` | `aria-colindex` | Col number. Needed for Virtual columns and Hidden columns scenarios. |
| `.k-grid-header-wrap>table>thead>tr:not(.k-filter-row)>th` | `aria-haspopup` = `dialog` | Present if the column has a ColumnMenu or FilterMenu. |
...
| `.k-grid:not(.k-grid-lockedcolumns) .k-grid-content>table>tbody>tr, .k-grid-pinned-container tbody tr` | `aria-rowindex` | Row number. Needed for Paging, Virtual rows, or Master/Detail rows scenarios. |
...
| `.k-grid:not(.k-grid-lockedcolumns) .k-grid-content>table>tbody>tr>td, .k-grid-pinned-container tbody td` | `aria-colindex` | Col number. Needed for Virtual columns and Hidden columns scenarios. |
...
| `tfoot tr` | `aria-rowindex` | Row number. Needed for Paging, Virtual rows, or Master/Detail rows scenarios. |
| `tfoot tr td` | `role` = `gridcell` | Required as the owner <table> element has its semantic role removed. |
| `tfoot tr td` | `aria-colindex` | Col number. Needed for Virtual columns and Hidden columns scenarios. |
...
| `.k-grid-sticky-container td` | `aria-colindex` | Col number. Typically 1 for the group header cell spanning all columns. |
...
| `.k-grid-pinned-container td` | `aria-haspopup` = `menu` | Pinned row cells announce the context menu for row pinning operations. |
...
| `.k-grid-content td` | `aria-haspopup` = `menu` | Regular data cells announce context menu when row pinning uses contextual mode. |
