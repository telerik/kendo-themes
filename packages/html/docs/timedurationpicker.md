...
| `.k-timedurationpicker .k-input-inner` | `aria-controls (when open)` | Points to the popup element. |
| `.k-timedurationpicker .k-input-inner` | `label for or aria-label or aria-labelledby` | The input needs an accessible name. |
...
| `.k-timedurationpicker.k-disabled .k-button` | `disabled or aria-disabled` | Rendered only when the picker is disabled. |
