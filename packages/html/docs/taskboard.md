...
| `.k-taskboard-column-header-actions > .k-button` | `role` = `button` | The TaskBoard Column actions are buttons. |
| `.k-taskboard-column-header-actions > .k-button` | `aria-label` / `title` | Each action must have an accessible name as they are represented by icons. |
...
| `.k-taskboard-card-menu-button` | `role` = `button` | The TaskBoard card menu element must be a button. |
| `.k-taskboard-card-menu-button` | `aria-label` / `title` | The menu button must have an accessible name as it is represented by an icon. |
| `.k-taskboard-pane-header-actions > .k-button` | `role` = `button` | The TaskBoard edit form close element must be a button. |
| `.k-taskboard-pane-header-actions > .k-button` | `aria-label` / `title` | The edit form close button must have an accessible name. |
