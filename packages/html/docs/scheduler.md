...
| `.k-scheduler-toolbar` | `aria-label` / `title` | Follows Toolbar spec: each toolbar must have a label specifying its purpose. |
...
| `.k-scheduler-views .k-segmented-control-button` | `aria-pressed` | Follows SegmentedControl spec: specifies the current state of the view SegmentedControl. Only the selected button within the group will have this attribute set to true. |
