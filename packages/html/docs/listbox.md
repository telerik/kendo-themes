...
| --- | --- |
| `role="listbox"` | Specifies the role of the ListBox list element. |
| `role="option"` | Specifies the role of the ListBox item element. |
| `aria-selected="true"|\"false"` | Set to true if the item is selected. |
| `tabindex` | The focused listbox item should have tabindex 0, all others -1. |
| `role="toolbar"` | The toolbar is a collection of command buttons. |
| `aria-controls` | Points to the id of the listbox element being controlled. |
| `aria-multiselectable` | when multiple selection is enabled |
| `id` | ID for the listbox element; referenced by the toolbar aria-controls. |
