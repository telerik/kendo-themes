...
| --- | --- | --- |
| `.k-fab` | `role` = `button` | Omitted if the <button> DOM element is used. |
| `.k-fab` | `aria-label` / `title` | The attribute must be present when no text is initially visible in the button. |
