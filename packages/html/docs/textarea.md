...
| `aria-label|aria-labelledby` | The textarea requires an accessible name (provided by consuming app). |
| `aria-multiline="true"` | Required on role=textbox to indicate multi-line behavior. Implicit on native `<textarea>`, so not needed there. |
