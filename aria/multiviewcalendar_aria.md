...
| `.k-calendar-thead` | `role=rowgroup` | The `k-calendar-thead` elements must explicitly have their roles set to `rowgroup` as their semantics have been removed while setting their `<table>` elements role to `none`. |
...
| Selector | Attribute | Usage |
| -------- | --------- | ----- |
| `.k-calendar-nav-prev,.k-calendar-nav-next` | `tabindex=-1` | The buttons must not be focusable. |
| `.k-calendar-title` | `tabindex=-1` | The button must not be focusable. |
| `.k-calendar-nav-today` | `tabindex=-1` | The link must not be focusable. |
