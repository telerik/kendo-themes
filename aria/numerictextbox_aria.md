...
| -------- | --------- | ----- |
| `.k-numerictextbox input.k-input-inner` | `role=spinbutton` | Announces the spin button capabilities of the NumericTextBox. |
...
| `.k-numerictextbox.k-disabled input.k-input-inner` | `disabled=disabled` or `aria-disabled=true` | The attribute is rendered only when the NumericTextBox is disabled. |
