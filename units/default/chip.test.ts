...
describe(`${component} CSS specificity`, () => {
  ...
  const chipSelectors = getSelectorsSpecificity(result.css, {
    filter: Chip.className,
    minSpecificity: 0,
    sourceMap: result.sourceMap,
  });

  chipSelectors.forEach((selectorInfo) => {
    const { selector, specificityValue, sourceLocation } = selectorInfo;
    const expectedSpecificity = calculateSpecificityThreshold(selector, Chip);
...
testKendoComponent(component, group, Chip.className, dependencyClassNames, [...expected, ...unexpected]);
