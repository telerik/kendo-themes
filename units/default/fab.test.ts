...
describe(`${component} CSS specificity`, () => {
  ...
  const fabSelectors = getSelectorsSpecificity(result.css, {
    filter: FloatingActionButton.className,
    minSpecificity: 0,
    sourceMap: result.sourceMap,
  });

  fabSelectors.forEach((selectorInfo) => {
    const { selector, specificityValue, sourceLocation } = selectorInfo;
    const expectedSpecificity = calculateSpecificityThreshold(selector, FloatingActionButton);
...
testKendoComponent(component, group, FloatingActionButton.className, dependencyClassNames, [...expected, ...unexpected]);
